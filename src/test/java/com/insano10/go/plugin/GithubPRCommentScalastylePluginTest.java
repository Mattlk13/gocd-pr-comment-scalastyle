...
public class GithubPRCommentScalastylePluginTest
{
    ...
    private static final String ARTIFACT_FILE_LOCATION = "analysis";
    ...
    {
        final GoPluginApiResponse response = plugin.handle(executeTaskFromMasterRequest(WORKING_DIRECTORY, RESULTS_FILE_LOCATION, ARTIFACT_FILE_LOCATION));
    ...
    {
        final GoPluginApiResponse response = plugin.handle(executeTaskFromPRRequest(REPOSITORY_URL, PULL_REQUEST_ID, WORKING_DIRECTORY, UNKNOWN_RESULTS_FILE_LOCATION, ARTIFACT_FILE_LOCATION));
    ...
    {
        final String expectedTrackbackLink = "http://localhost:8153/go/files/my-test-pipeline/19/defaultStage/1/myJob/analysis/scalastyle-result.xml";
        final String expectedComment = "hello world!";

        when(scalastyleResultsAnalyser.buildGithubMarkdownSummary(RESULTS_FILE_PATH, expectedTrackbackLink)).thenReturn(expectedComment);

        final GoPluginApiResponse response = plugin.handle(executeTaskFromPRRequest(REPOSITORY_URL, PULL_REQUEST_ID, WORKING_DIRECTORY, RESULTS_FILE_LOCATION, ARTIFACT_FILE_LOCATION));
    ...
    {
        final DefaultGoPluginApiRequest request = executeTaskFromPRRequest(REPOSITORY_URL, PULL_REQUEST_ID, WORKING_DIRECTORY, RESULTS_FILE_LOCATION, ARTIFACT_FILE_LOCATION);
